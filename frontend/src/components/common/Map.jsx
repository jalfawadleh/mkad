...
const ORGANISATION_MARKER_ICON = new L.divIcon({
  html: ICON_ORGANISATION,
...
const ACTIVITY_MARKER_ICON = new L.divIcon({
  html: ICON_ACTIVITY,
  iconSize: new L.Point(35, 35),
  iconAnchor: new L.Point(10, 35),
  popupAnchor: new L.Point(8, -35),
...
const MKADIFFERENCE_MARKER_ICON = new L.divIcon({
  html: ICON_MKADIFFERENCE,
...
// NOTE: iconCreateFunction is running by leaflet, which does not support arrow syntax.
function createClusterActivitiesIcon(cluster) {
  return L.divIcon({
    html: `
...
function createClusterOrganisationsIcon(cluster) {
  return L.divIcon({
    html: `
      <div class="d-flex justify-content-center h-100 border-1 border-primary rounded-circle"
...
function createClusterMembersIcon(cluster) {
  return L.divIcon({
    html: `
      <div class="d-flex justify-content-center h-100 border-1 border-primary rounded-circle"
...
const Map = () => {
  ...
  const [mapCenter, setMapCenter] = useState({ lat: user.lat, lng: user.lng });
  ...
  const getMapItems = useCallback(async (location, signal) => {
    if (!location) return;
    const { lat, lng } = location;
    if (lat == null || lng == null) return;

    try {
      const res = await axios.post("/map", { lat, lng }, { signal });
      setItems(res.data);
    } catch (error) {
  ...
  const clearFlyToLocation = useCallback(() => setFlyToLocation(null), []);

  const Recenter = ({ flyToLocation, clearFlyToLocation }) => {
    ...
    useEffect(() => {
      map.eachLayer((layer) => {
        if (layer instanceof L.TileLayer && layer.getContainer) {
          ...
          if (!container) return;
          if (darkmode) {
            container.classList.add("map-darkmode");
            container.classList.remove("map-lightmode");
          } else {
            container.classList.add("map-lightmode");
            container.classList.remove("map-darkmode");
          }
        }
      });
    }, [darkmode, map]);
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      getMapItems(mapCenter, controller.signal);
    }, 250);
  ...
  const MarkerMemberInner = ({ item, ItemPopup }) => {
    const icon = useMemo(
      () =>
        new L.icon({
    ...
    return (
      <Marker key={item._id} position={[item.lat, item.lng]} title={item.name} icon={icon}>
        <ItemPopup item={item} />
      </Marker>
    );
  };
  ...
  const MarkerMember = ({ item }) => (
    <MarkerMemberInner item={item} ItemPopup={ItemPopup} />
  );
    ...
    <>
      <MapContext.Provider value={{ setFlyToLocation }}>
        ...
        >
          ...
          />
          <ApplyMapTheme darkmode={user.darkmode} />
          ...
          >
            ...
            <Marker
              ...
              icon={MKADIFFERENCE_MARKER_ICON}
              title='MKaDifference'
            >
              <Popup closeButton={false}>
                <span
